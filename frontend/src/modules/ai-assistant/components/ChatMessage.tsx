...
import {
	formatDateRange,
	getPendingUpdateFields,
} from '../utils/ai-assistsnt-utils'
...
export function ChatMessage({
	...
	const hasPendingTasks = !isUser && !!message.pendingTasks?.length
	const hasPendingUpdate = !isUser && !!message.pendingUpdate
	const hasPendingAction = hasPendingTask || hasPendingTasks || hasPendingUpdate
	...
	return (
		...
		>
			...
			{hasPendingTasks && message.pendingTasks && (
				<div className='ml-1 w-full max-w-[88%] rounded-md border border-border bg-muted/40 px-3 py-2'>
					<p className='mb-1.5 text-xs font-medium text-foreground'>
						Задачи ({message.pendingTasks.length})
					</p>
							...
							<div key={idx} className='text-xs'>
								...
								{(task.startAt || task.endAt) && (
									<span className='ml-1.5 text-muted-foreground'>
										— {formatDateRange(task.startAt, task.endAt, task.isAllDay)}
									</span>
								)}
							</div>
						))}
