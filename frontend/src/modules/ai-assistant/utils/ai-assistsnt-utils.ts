...
export function getPendingUpdateFields(update: PendingUpdate): UpdateField[] {
	...
	if (update.status !== null) {
		fields.push({
			label: 'Статус',
			value:
				update.status === TaskStatus.Completed ? 'Выполнена' : 'В процессе',
		})
	}

	if (update.clearNotifyAt) {
		fields.push({ label: 'Напоминание', value: 'Убрать' })
	} else if (update.notifyAt !== null) {
		fields.push({
			label: 'Напоминание',
			value: format(new Date(update.notifyAt), 'd MMMM yyyy, HH:mm', {
				locale: ru,
			}),
		})
	}
