...
export function TaskList({
	...
	const shouldUseExternalData = tasksProp !== undefined || isLoadingProp !== undefined || errorProp !== undefined;
	...
	const handleFetchMoreTasks = async () => {
		if (shouldUseExternalData || hasMore === false || isLoading || isLoadingMore) {
			return;
		}
	...
	if (resolvedError) {
		return <p className="py-8 text-center text-sm text-destructive">{resolvedError}</p>;
	}
