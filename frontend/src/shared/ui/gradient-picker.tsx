...
export function GradientPicker({
	...
	const solids = ["#E2E2E2", "#ff75c3", "#ffa647", "#ffe83f", "#9fff5b", "#70e2ff", "#cd93ff"];
			...
			<PopoverTrigger asChild>
				...
				>
					{showValue ? (
						...
						<div
							className="h-full w-full rounded-[inherit] transition-colors"
							style={{ backgroundColor: background }}
						/>
					)}
				</Button>
			</PopoverTrigger>
			<PopoverContent className={cn("w-64", hexOnly && "w-80")}>
				{hexOnly ? (
					<div className="space-y-4">
						<div className="flex flex-wrap gap-1">
							{solids.map((solidColor) => (
								<button
									key={solidColor}
									type="button"
									style={{ backgroundColor: solidColor }}
									className="h-6 w-6 rounded-md cursor-pointer active:scale-105"
									onClick={() => setBackground(solidColor)}
								/>
							))}
						</div>
						<ColorPickerContent
					...
					<TabsContent value="solid" className="flex flex-wrap gap-1 mt-0">
						...
						<Popover>
							<PopoverTrigger asChild>
								<div
									className="rounded-md h-6 w-6 cursor-pointer active:scale-105 border-2 border-dashed border-muted-foreground/25 hover:border-muted-foreground/50 transition-colors flex items-center justify-center"
									title="Custom color"
								>
							...
							<PopoverContent className="w-80" side="left">
								<div className="space-y-3">
									<div className="space-y-1">
										...
										<p className="text-xs text-muted-foreground">Choose your custom color</p>
									</div>
									<ColorPickerContent
										color={customColor}
										onChange={(color) => {
